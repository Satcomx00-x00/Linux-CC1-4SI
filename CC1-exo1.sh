...
createFolders() {
    for folder in "${foldersToCreate[@]}"; do # pour chaque nom et prénom, créer un dossier avec le nom et le prénom (col2 _ col3)
        mkdir -p "$3/$1 $2/$folder"           # créer les dossiers avec le nom et le prénom (col2 _ col3) et créer des sous-dossiers 'Français', 'Histoire', 'Mathematiques', 'Docs'.
...
do
    if [[ $col1 =~ [0-9] ]] || [[ $col2 =~ [0-9] ]] || [[ $col3 =~ [0-9] ]]; then # vérifier s'il y a un numéro dans l'une des colonnes et si c'est le cas
        ...
        createFolders "$col2" "$col3" "$classe" # appeler la fonction createFolders avec les valeurs de col2, col3 et classe
    fi
